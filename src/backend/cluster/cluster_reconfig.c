...
cluster_reconfig_shmem_init(void)
{
	...
	if (!found) {
		...
		/* RF-ROOT P9 audit #2 redo part 3 (DSH B′): bootstrap publication
		...
		/* RF-ROOT P9 audit #2 redo part 3 (DSH B′): cold-formation marker
...
/*
 * cluster_reconfig_bootstrap_proof_node — RF-ROOT P9 audit #2 redo part 3 /
 * DSH B′ ruling: the WHOLE-round founding-formation proof for ONE target
...
{
	/* DSH B′ ruling: bounded retries only — this runs under the reconfig
...
/*
 * RF-ROOT P9 audit #2 redo part 3 / DSH B′ cold-formation ruling: true only
	...
	if (ReconfigShmem != NULL) {
		...
		for (i = 0; i < CLUSTER_MAX_NODES; i++) {
			...
			if (dead_bitmap_test_bit(dead_bitmap, i))
				...
				/*
				 * RF-ROOT P9 audit #2 redo part 3 / DSH B′ ruling
				 ...
				 * cold-formation path (DSH B′ cold-formation ruling): a
